...
"""
> plot.per-strain.abs_values.dep.py <

Uses matplotlib to plot a linear regressions for CC7, H2 and RS, and restrict
genes of interest to those which are differently expressed at protein level.
"""
...
for n, strain in enumerate(['CC7', 'H2', 'RS']):
    # filter out prots that are not differentially expressed
    # by reading ../gene_lists/{}_prot_diff.txt
    dep = open('../gene_lists/{}_prot_diff.txt'.format(strain.lower())).read()
    dep = dep.split('\n')[:-1]      # [:-1] removes [..., ''] at the end
    
    sub_data = data[data['gene_id'].isin(dep)]
    
    # read data from both xx-25.tsv and xx-32.tsv files
    sub_data = sub_data.filter(regex=strain)
...
output_filename = 'per-strain.fc_values.dep.pdf'
